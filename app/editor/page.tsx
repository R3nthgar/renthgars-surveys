...
export default function Home() {
  ...
  const createSurvey = ({
    ...
    reset ? update({}) : setCreatingSurvey(false);
  };
    ...
    return (
      <div className="ql-editor" style={{ padding: 0 }}>
        {!user.current ? (
                ...
                {Object.values(user.current.surveys).map((curSurvey) => (
                  <button
                    ...
                    onClick={async () => {
                      ...
                      categories.current = Object.keys(
                        survey.current.categories
                      );
                      update({});
                    }}
                  >
          ...
          <>
            <div className="groupEditor topbar">
              <div>
                <button
                  className="borderer"
                  onClick={() => {
              ...
              <div>
                <button
                  className="borderer"
                  onClick={() => {
                    previewing.current = !previewing.current;
                    update({});
                  }}
                >
                  Preview
                ...
                <button
                  ...
                  onClick={async () => {
                    if (survey.current) {
                      doResults.current = !doResults.current;
                      if (doResults.current) {
                        const tempResults = await getData(
                          "results",
                          survey.current.ID
                        );
                        if (doResults.current && tempResults) {
                ...
                <button
                  ...
                  onClick={async () => {
                    if (survey.current)
                      try {
                        await navigator.clipboard.writeText(survey.current.ID);
                        console.log("Text copied to clipboard");
                      } catch (err) {
                        console.error("Failed to copy text: ", err);
                      }
                  ...
                  <>
                    ...
                    {takenSurvey != 0 ? (
                      <>
                        <CustomChart
                          ...
                          data={Object.values(results.current || {}).map(
                            (value) =>
                    ...
                    {takenSurvey != 0 ? (
                      survey.current.questions
                        .filter(
                          (val) =>
                            val.questions.filter(({ type }) => type != "DIS")
                              .length
                        )
                        .map((group, index) => (
            ...
            {overpage?.type == "importing" ? (
                            ...
                            if (evt.target && evt.target.result) {
                              ...
                              const data = lines.map((line) => {
                                return match(line).reduce((acc, cur, i) => {
                                  const val =
                                    cur.length <= 0 ? null : Number(cur) || cur;
                                ...
                                data.map((val, index) => {
                                  if (overpage?.type == "importing") {
                                    ...
                                    if (
                                      ...
                                      } = { option: val.Text, categories: {} };
                                      delete val.Text;
                                      delete val["Question Type"];
                                      option.categories = Object.entries(val)
                                        .filter(([key, val]) => val)
                                        .reduce(
                                          (
                                      ...
                                      group.questions.push({
                                        type: translator[val["Question Type"]],
                                        name: val.Text,
                                        values: [],
                                      });
                                    }
                    ...
                    onClick={async () => {
                      if (
                        ...
                        document
                          ...
                          .namedItem("deleter")?.value == survey.current.ID &&
                        user.current
                      ) {
              ...
              <>
                <div style={{ textAlign: "center" }}>
                  ...
                  causing issues. It&apos;s recommended to download and clear
                        ...
                        if (survey.current && user.current) {
                          ...
                          addDoc(
                            ...
                            },
                            {
                              collection: "rules",
                              doc: survey.current.ID,
                              data: JSON.parse(JSON.stringify(survey.current)),
                            },
                            {
                              collection: "results",
                              doc: survey.current.ID,
